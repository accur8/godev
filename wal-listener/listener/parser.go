...
func (p *BinaryParser) ParseWalMessage(msg []byte, tx *WalTransaction) error {
	...
	case BeginMsgType:
		...
		log.Debug(
			"begin type message was received %s %s",
	...
	case CommitMsgType:
		...
		log.Debug(
			"commit message was received %s %s",
	...
	case OriginMsgType:
		log.Debug("origin type message was received")
	case RelationMsgType:
		relation := p.getRelationMsg()

		log.Debug(
			"relation type message was received %s %s",
	...
	case TypeMsgType:
		log.Debug("type message was received and ignored")
	case TruncateMsgType:
		log.Debug("truncate message was received and ignored")
	case InsertMsgType:
		insert := p.getInsertMsg()

		log.Debug(
			"insert type message was received %s",
	...
	case UpdateMsgType:
		...
		log.Debug("update type message was received %s", slog.Any("relation_id", upd.RelationID))
	...
	case DeleteMsgType:
		...
		log.Debug(
			"delete type message was received %s",
...
func (p *BinaryParser) readTupleData() []TupleData {
	...
	for i := 0; i < size; i++ {
		...
		switch sl[0] {
		case NullDataType:
			log.Debug("tupleData: null data type")
		case ToastDataType:
			log.Debug("tupleData: toast data type")
		case TextDataType:
