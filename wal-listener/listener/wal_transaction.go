...
func NewWalTransaction(monitor transactionMonitor) *WalTransaction {
...
func (w *WalTransaction) CreateEventsWithFilter(tableMap map[string][]string) []publisher.Event {
	...
	for _, item := range w.Actions {
		...
		if true || (validTable && validAction) {
		...
		log.Info(
			"wal-message was skipped by filter %s %s %s",
